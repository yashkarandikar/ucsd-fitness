...
def remove_outliers(data, params, param_indices, scale_factors):
    ...
    c = param_indices["pace(avg)"]; cols.append(c); lower_bounds.append(0.0); upper_bounds.append(200.0)
...
def prepare(infile, outfile, mode):
    sport = "Running"
    params = ["user_id","Distance", "pace(avg)", "date-time"]
...
if __name__ == "__main__":
    ...
    [mse, var, fvu, r2, errors] = compute_stats(train_set[:, param_indices["pace(avg)"]], train_pred)
    ...
    [mse, var, fvu, r2, errors] = compute_stats(val_set[:, param_indices["pace(avg)"]], val_pred)
