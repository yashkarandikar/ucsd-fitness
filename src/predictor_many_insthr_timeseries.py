...
def make_predictions(data, models, param_indices):
    ...
    while w < N:
        ...
        while w < N and data[w, 0] == u:
            w += 1
            i += 1
        tpred = tpred + model.predict(start = 0, end = i - 1)
    return np.matrix(tpred).T
...
def learn(data, param_indices):
    hr_ind = param_indices["hr"]
    dt_ind = param_indices["duration"]
    N = data.shape[0]
    W = get_workout_count(data)
    ...
    if (type(data).__name__ == "matrix"):
        col0 = data[:, 0].A1
        all_hr = data[:, hr_ind].A1
        all_dt = data[:, dt_ind].A1
    else:
        col0 = data[:, 0]
        all_hr = data[:, hr_ind]
        all_dt = data[:, dt_ind]
    w_indices = list(np.searchsorted(col0, wins))
    w_indices.append(N)
    P = 10
    for w in xrange(0, W):
        w_start = w_indices[w]
        w_end = w_indices[w + 1]
        hr = all_hr[w_start : w_end]
        dt = all_dt[w_start : w_end]
        dt = [datetime.fromtimestamp(t) for t in dt]
        X = np.zeros((len(dt) - P, P))
        y = np.zeros(len(dt) - P)
        for p in xrange(0, len(dt) - P):
            X[p, :] = hr[p : p + P]
            y[p] = hr[p + P]
...
if __name__ == "__main__":
    ...
    print "Training.."
    model = learn(train_set, param_indices)
    ...
    train_pred = make_predictions(train_set, model, param_indices)
    val_pred = make_predictions(val_set, theta, E, param_indices, use_features)
    print param_indices
