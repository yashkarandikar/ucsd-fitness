...
void learn(char *infile, double lam1, double lam2, char* outfile, int E, int lbfgs_max_iterations, bool use_features)
{
    ...
    lbfgsparam.m = 10;
    lbfgsparam.max_iterations = lbfgs_max_iterations;
    ...
    */
    ...
    ofstream of;
    string outfile_E = "E_" + string(outfile);
    string outfile_theta = "theta_" + string(outfile);
    string outfile_sigma = "sigma_" + string(outfile);
    
    of.open(outfile_E);
    of << E << "\n";
    of.close();

    of.open(outfile_theta);
    int i = 0;
    ...
    of.open(outfile_sigma);    
    int n_workouts = sigma.size();
    vector<int> sigma_u;
    for (int i = 0 ; i < n_workouts; i++) {
        of << "[";
        sigma_u = sigma[i];
        int j = 0;
