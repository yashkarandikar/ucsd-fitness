...
We have one parameter $\alpha_u$ per runner $u$. The parameters $\alpha, \theta_0, \theta_1$ are common to all runners. Thus, we have a total of $U + 3$ parameters, where $U$ is the number of distinct runners in the data set. These parameters are learned during training.
...
$\lambda$ is the regularization hyper-parameter, which we select by performing a line search over values in the set $\{0, 10^{-4}, 10^{-3},...,10^3, 10^{4}, 10^5\}$ and choosing the $\lambda$ which minimizes the error on a validation set. We find the $\Theta$ which satisfies the objective using the L-BFGS \cite{lbfgs} implementation available in SciPy \cite{scipy}.
...
where $e_{ui}$ is the experience level of the runner $u$ at the $i$'th workout. Thus, we have one parameter $\alpha_{ue}$ per runner $u$ per experience level $e$. Further, we have an intercept term $\alpha_e$ for every experience level $e$, common to all runners. The terms $\theta_0$ and $\theta_1$ are common to all runners and experience levels. Thus, given $U$ runners and $E$ experience levels, we have a total of $UE + E + 2$ parameters.
...
where $D$ is the data set of all workouts for all runners. $T_{ui}$ is the true duration of workout $i$ of runner $u$. This objective is similar to the one described in \cite{www13}. $\Omega_1$ and $\Omega_2$ are regularizers defined as follows:
...
For the task of predicting the average heart rate instead of the duration of the workout, the objective function remains the same as in equation \ref{eqnObjective}, except that the term $T_{ui}' - T_{ui}$ is replaced by $H_{ui}' - H_{ui}$. $H_{ui}$ and $H_{ui}'$ are the true and predicted average heart-rates respectively.
...
Given the instantaneous heart-rate at various instants in a workout, we associate a \emph{tiredness level} $e_{wi}$ with heart-rate sample $i$ in the workout $w$. As before, the value $e_{wi}$ is an integer in the interval $[1, E]$, where $E$ is the number of tiredness levels. We expect the tiredness level of the runner to either stay the same or increase with each sample in the workout. We encode this in the form of a monotonicity constraint as before:
...
where $D$ is the data set of all samples in all workouts. $h_{wt}$ is the true instantaneous heart-rate at sample $t$ in workout $w$. This objective is similar to the one described in \cite{www13}.
...
The lack of improvement in the prediction accuracy after accounting for temporal evolution across workouts seems to suggest that we need data for many more workouts per runner in order to be able to notice a reasonably strong evolution of the runner's fitness level across workouts. Further, an increase in the runner's physical capacity might also be exhibited through the fact that the runner was able to complete a workout of longer duration or larger distance and this may not necessarily reflect through a change in the relationship between the duration (or average heart-rate) and the distance. This kind of temporal evolution would not be visible as an increase in the $R^2$ score in the prediction of the duration or average heart-rate.
...
The code for this work can be found online on GitHub at:	  \url{https://github.com/yashkarandikar/ucsd-fitness}
