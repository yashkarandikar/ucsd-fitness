...
\begin{enumerate}
\item Given the first $n$ instantaneous distance and heart-rate values in a run and given the $(n+1)$'th distance value, predict the $(n+1)$'th instantaneous heart-rate value within the same run.
\item Given the instantaneous distance and heart-rate values for the first $f$ fraction of a run and given the distance values for the remaining $1-f$ fraction of the run, predict the instantaneous heart-rate values for the remaining part of the run. This task is a generalization of the first.
\end{enumerate}
...
$$\hat{\Theta} = \arg\min_{\Theta}\frac{1}{|D|} \sum_{T_{ui} \in D}(T_{ui}' - T_{ui})^2 + \lambda (\theta_1^2 + \sum_{u}{\alpha_u^2}) $$

where $D$ is the data set of all workouts for all runners, $T_{ui}$ is the true distance of workout $i$ of runner $u$. The term $\lambda \sum_{u}{\alpha_u^2} $ is a regularizer. $\lambda$ is the regularization hyper-parameter, which we select by performing a line search over values in the set $\{0, 10^{-5}, 10^{-4},...,10^3, 10^{4}\}$ and choosing the $\lambda$ which minimizes the error on a validation set. We find the $\Theta$ which satisfies the objective using the L-BFGS \cite{lbfgs} implementation available in SciPy \cite{scipy}.
...
Let $\beta$ denote the set of all experience levels for all workouts of all runners. Then, we need to choose the optimal parameters $(\hat{\Theta}, \hat{\beta})$ according to the objective
...
Let $\beta$ denote the set of tiredness levels for all samples in all workouts. Then, we need to choose the optimal parameters $(\hat{\Theta}, \hat{\beta})$ according to the objective
...
Baseline& 0.520913 & 0.422149 & 0.423354  \\ \hline
...
& Training $R^2$ & Validation $R^2$ & Test $R^2$ \\ \hline
\# Examples & 39545 & 2384  &  2384 \\ \hline
Baseline  & 0.517636 & 0.480985 & 0.430890 \\ \hline
